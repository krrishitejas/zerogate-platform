...
func (a *Agent) Start() error {
	...
	log.Println("Bug Detection Agent started, listening for 'graph.extracted'")
	return nil
}
...
func (a *Agent) handleGraphExtracted(msg *nats.Msg) {
	...
	for _, node := range event.Nodes {
		if node.Label == "File" {
			...
			// Phase 1: Regex-based fast scan
			results, err := scanner.ScanFileForBugs(fullPath)
			if err != nil {
				continue
			}
			...
			// Phase 2: LLM deep analysis (if available and file has potential issues or is complex)
			if useLLM {
				llmResults, err := scanner.ScanFileWithLLM(fullPath, "bug", "")
				if err != nil {
					log.Printf("[Bug Agent] LLM analysis failed for %s: %v", node.Name, err)
					continue
				}
...
func (a *Agent) publishFinding(projectID, filePath string, res scanner.FindingResult) {
	...
	log.Printf("[Bug Agent] %s finding for %s (Rule: %s, Source: %s)", res.Severity, filePath, res.RuleID, res.Source)
}
